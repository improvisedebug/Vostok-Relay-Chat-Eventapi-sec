...
# =============================================================================
# Vostok Relay Chat -- MCM Config Registration
# =============================================================================
...
func _ready() -> void:
	...
	config.set_value("Bool", "auto_connect", {
		...
		"tooltip" = "Open the WebSocket as soon as the game loads. Combined with Stealth Mode this makes messages accumulate silently in the background so they're already there when you open the chat window. Disable to keep the mod fully offline until you click Connect.",
		"default" = true,
		"value"   = true,
		"category" = "Connection"
	})
	...
	config.set_value("Bool", "connect_on_open", {
		"name"    = "Connect Only While Chat Open (DEPRECATED)",
		"tooltip" = "DEPRECATED in v1.7.9 -- this toggle no longer does anything. Disconnecting whenever the chat window closed silently dropped every incoming message in between, so the mod now always stays connected for the whole game session (subject to Auto-Connect On Boot + AFK auto-disconnect). To go fully silent without dropping the socket, leave 'Stealth Mode' ON.",
		"default" = false,
		"value"   = false,
		"category" = "Connection"
	})
	...
	config.set_value("Bool", "stealth_when_closed", {
		...
		"tooltip" = "Socket stays connected the whole game session, but while the chat window is CLOSED no outgoing chat, autobroadcasts, or mod events are sent. Incoming messages still arrive and accumulate in the chat history so you see them when you open the window. Default ON -- this is what gives you 'show me what I missed' behavior. Overrides 'Connect Only While Chat Open'.",
		"default" = true,
		"value"   = true,
		"category" = "Connection"
	})
	...
	config.set_value("Bool", "suppress_join_leave", {
		...
		"tooltip" = "Never send the 'join' or 'leave' system messages. Other players will not see you connect or disconnect. Recommended ON when using Stealth Mode (default ON). Note: the relay still knows you connected -- only other players are blind to it.",
		"default" = true,
		"value"   = true,
		"category" = "Connection"
	})
	...
	config.set_value("Bool", "event_respond_enabled", {
		"name"    = "Respond to /event Polls",
		"tooltip" = "When ON, other players who type /event in chat will receive a one-line summary of YOUR currently-enabled event templates (the same lines that are already configured for broadcast). Replies are rate-limited to one per 60s. When OFF, your client silently ignores all /event polls -- you are indistinguishable from someone who has no events configured.",
		"default" = true,
		"value"   = true,
