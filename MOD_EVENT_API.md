# Vostok Relay Chat — Mod Event API

> Audience: mod authors who want to broadcast in-game events from their own mod through Vostok Relay Chat.
> Mod: **Vostok Relay Chat v1.8.7+** (internal id still `SafeRelayChat`, pack file `950-SafeRelayChat.vmz`)
...
Vostok Relay Chat exposes a small public API that lets other mods:
...
3. Have each registered event automatically surface in **MCM → Vostok Relay Chat → Mod Event Hooks** as two settings the user controls:
...
- **AFK auto-disconnect** — off by default since v1.8.4 (the always-on connection model means we don't silently disconnect on input idle). If the user explicitly enables it via `/afk on` or MCM, then after the configured idle timeout (default 15 min) the relay disconnects and `post_registered_event` returns `false` until they reconnect.
...
func _ready() -> void:
    ...
    if relay == null:
        return  # Vostok Relay Chat not installed — silently no-op
...
After one game restart following `register_event_category`, the user sees a new section under **Vostok Relay Chat**:
...
Players have a few debug commands for the registry:

| Command | Effect |
|---------|--------|
| `/listevents` | Lists every registered event slug with its on/off state. |
| `/event` | Polls the relay for event summaries from other players. Each peer running this mod with the responder enabled replies with a one-line summary of their currently-enabled event templates. The local client opens a 5 s collection window, then renders the responses (deduped by username). Rate-limited to once per 30 s per sender; each peer auto-throttles to one reply per 60 s. |
| `/respond on\|off` | Toggle whether the local client replies to incoming `/event` polls (default ON; also `MCM → Mod Event Hooks → Respond to /event Polls`). When OFF, the client silently ignores polls — a player with the responder off is indistinguishable from a player who has no events configured. |
...
- Only **enabled** templates are shared. Disabled ones are never broadcast.
- Templates pass through the same outgoing filter pipeline as chat (link/PII strip, optional profanity). A template that contains a link or path will cause the entire reply to be dropped before send.
- Incoming replies run through the same inbound hardening as chat (Unicode sanitization, length cap, link/path drop, optional profanity censor).
- Replies use a new `event_reply` packet type. Vanilla (non-mod) clients on the same relay ignore it.
