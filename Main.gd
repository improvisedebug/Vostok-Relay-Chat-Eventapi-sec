...
var _auto_connect     : bool   = true   # default ON so background message collection works out of the box
var _ephemeral_identity : bool = true
# v1.8.4: _connect_on_open removed -- socket is now always-on for the whole
# game session once auto-connect fires. Drop paths: /disconnect command,
# kill switch (-), AFK timeout, network_enabled flipped off, or quit.
var _stealth_closed   : bool   = true   # default ON: stay connected while window closed; mute outgoing.
var _suppress_jl      : bool   = true   # default ON to avoid noisy join/leave churn from open/close toggling

# AFK auto-disconnect. v1.8.4: default OFF so always-on behavior isn't
# silently undone after 15 min of input-quiet (you may be just walking).
var _afk_enabled        : bool    = false
...
# Hashes of recently-sent texts (within ~6s) so the relay's echo of our own
# message doesn't render twice. Cleared on each new send.
var _recent_self_sends : Array = []
# Traffic accounting (local-only, never broadcast). Lifetime + last-window
# byte counts so we can show data totals and rates in the chat.
var _net_bytes_in       : int   = 0
var _net_bytes_out      : int   = 0
var _net_msgs_in        : int   = 0
var _net_msgs_out       : int   = 0
var _net_win_bytes_in   : int   = 0
var _net_win_bytes_out  : int   = 0
var _net_win_start_t    : float = 0.0
var _net_stat_interval  : float = 60.0
var _net_stat_next_t    : float = 60.0
var _net_stats_enabled  : bool  = true

# v1.8.4 self-test ping. /ping sends a tagged chat message and waits up to
# 5s for the relay to echo it back, then prints round-trip time. Gives the
# user a hard yes/no on "is the relay working right now".
var _ping_tag           : String = ""
var _ping_t0_ms         : int   = 0
var _ping_timeout_t     : float = 0.0

# v1.8.7 /event poll. /event broadcasts a small request to the relay; any
# peer running this mod with the responder enabled replies with a one-line
# summary of their currently-enabled event templates. The sender collects
# replies for 5s, deduped by username, then renders them in chat.
#
# Privacy: respond-side is opt-in (default ON, MCM-toggleable) and only
# enabled templates are shared -- which the user already opted into
# broadcasting. Both sides are rate-limited so /event cannot be used to
# spam the channel or fingerprint via timing.
var _event_respond_enabled : bool   = true
var _event_poll_tag        : String = ""
var _event_poll_replies    : Dictionary = {}  # username -> payload
var _event_poll_deadline   : float  = 0.0
var _event_req_next_t      : float  = 0.0   # local rate-limit on /event
var _event_resp_next_t     : float  = 0.0   # respond-side rate-limit
const EVENT_POLL_WINDOW_S  : float  = 5.0
const EVENT_REQ_COOLDOWN_S : float  = 30.0
const EVENT_RESP_COOLDOWN_S: float  = 60.0
const EVENT_REPLY_MAX_LEN  : int    = 240
const EVENT_REPLY_DICT_MAX  : int   = 64    # cap dedup dict so malicious relay can't OOM us during the 5s window
...
func _ready() -> void:
	...
	print("[SafeRelayChat] booting Vostok Relay Chat (v1.8.8) -- toggle key cached as keycode ", _toggle_keycode)
	...
	_migrate_v178_defaults()  # one-shot: flip existing MCM config to backlog-friendly defaults
	...
	# Hook quit paths. Autoload Nodes don't reliably receive
	# NOTIFICATION_WM_CLOSE_REQUEST, so we also bind to the root window's
	# close_requested signal and to the SceneTree's tree_exiting.
	var root_win := get_window()
	if root_win and not root_win.close_requested.is_connected(_on_app_close_requested):
		root_win.close_requested.connect(_on_app_close_requested)
	var tree := get_tree()
	if tree and not tree.tree_exiting.is_connected(_on_tree_exiting):
		tree.tree_exiting.connect(_on_tree_exiting)
	_net_win_start_t = Time.get_ticks_msec() / 1000.0
	_net_stat_next_t = _net_win_start_t + _net_stat_interval

	# v1.8.4 boot banner -- visible in the chat panel as soon as the user
	# opens it, so there's never a doubt that the mod loaded.
	_add_local_message("* Vostok Relay Chat v1.8.8 loaded. Type /ping to test the relay, /event to poll peers, /help for commands.",
		Color(0.55, 0.85, 1.0))
...
# ---- Config -----------------------------------------------------------------
# One-shot migration to v1.7.8 defaults: stay connected in the background so
# missed messages are visible the moment the chat window opens. Without this,
# users who upgrade from <=v1.7.7 keep the old "drop socket on close" config
# and never see a backlog. Marker file in user://SafeRelayChat/ ensures we
# only ever rewrite their config ONCE -- after that, whatever the user picks
# in MCM is respected.
func _migrate_v178_defaults() -> void:
	var marker := "user://SafeRelayChat/migrated_v178.flag"
	if FileAccess.file_exists(marker):
		return
	var ini_path := "user://MCM/SafeRelayChat/config.ini"
	if FileAccess.file_exists(ini_path):
		var cfg := ConfigFile.new()
		if cfg.load(ini_path) == OK:
			var changed := false
			for kv in [
				["connect_on_open", false],
				["stealth_when_closed", true],
				["auto_connect", true],
				["suppress_join_leave", true],
			]:
				var key: String = kv[0]
				var new_val: bool = bool(kv[1])
				var entry = cfg.get_value("Bool", key, null)
				if typeof(entry) == TYPE_DICTIONARY:
					entry["value"] = new_val
					cfg.set_value("Bool", key, entry)
					changed = true
				elif entry == null:
					cfg.set_value("Bool", key, {
						"value": new_val,
						"default": new_val,
					})
					changed = true
			if changed:
				cfg.save(ini_path)
				print("[SafeRelayChat] migrated MCM config to v1.7.8 backlog-friendly defaults")
	# Write marker regardless so we don't retry every boot.
...
func _apply_config_file(cfg: ConfigFile) -> void:
	...
	# v1.7.9: connect_on_open semantics removed. Disconnecting whenever the
	# chat window closed dropped every incoming message in between, defeating
	# the purpose of the relay. The socket now stays open for the whole game
	# session once auto-connect fires; outgoing is still muted while closed
	# (stealth_when_closed), and the only ways to drop the socket are the
	# /disconnect command, the kill switch (-), or AFK auto-disconnect.
	_auto_connect     = bool(_entry_value(cfg, "Bool",  "auto_connect",     true))
	_ephemeral_identity = bool(_entry_value(cfg, "Bool", "ephemeral_identity", true))
	# v1.8.4: connect_on_open MCM key fully removed from active config path.
	# Existing user configs are ignored (the MCM toggle is still labeled
	# DEPRECATED for clarity).
	_stealth_closed   = bool(_entry_value(cfg, "Bool",  "stealth_when_closed", true))
	_suppress_jl      = bool(_entry_value(cfg, "Bool",  "suppress_join_leave", true))
	_afk_enabled      = bool(_entry_value(cfg, "Bool",  "afk_disconnect_enabled", false))
	_afk_timeout_s    = float(clampi(int(_entry_value(cfg, "Int", "afk_timeout_min", 15)), 1, 120)) * 60.0
	_event_respond_enabled = bool(_entry_value(cfg, "Bool", "event_respond_enabled", true))
...
func apply_mcm_config(cfg: ConfigFile) -> void:
	...
	if _relay_url != old_url:
		...
		_disconnect_socket()
		if _has_privacy_ack() and _auto_connect:
...
func _show_privacy_notice() -> void:
	...
	panel.add_child(vb)
	var title := Label.new()
	title.text = "VOSTOK RELAY CHAT - PRIVACY NOTICE"
...
func _send_raw(data: Dictionary) -> void:
	...
	var payload_len : int = payload.length()
	if _meta_jitter > 0.0:
		...
		get_tree().create_timer(delay).timeout.connect(func():
			if is_instance_valid(self) and _connected and sock == _socket:
				...
				# Count only actually-sent bytes so dropped jittered sends
				# don't inflate the traffic readout.
				_net_bytes_out += payload_len
				_net_win_bytes_out += payload_len
				_net_msgs_out += 1)
		return
	_socket.send_text(payload)
	_net_bytes_out += payload_len
...
func _send_chat(text: String) -> void:
	...
	if _stealth_closed and not _open:
		...
		return
	_last_send_t = now
	# Echo locally BEFORE network send so the user always sees what they sent
	# (even if the relay drops the message or the socket buffers it).
	var safe_text: String = text.left(MAX_OUTGOING_TEXT)
	_add_chat_message(_username, safe_text)
	# Remember this send for ~6s so the relay's echo doesn't double-render it.
	_recent_self_sends.append({"text": safe_text, "t": now})
	while _recent_self_sends.size() > 8:
		_recent_self_sends.pop_front()
	_send_raw({"type": "chat", "username": _username, "text": safe_text})
...
func _handle_command(text: String) -> void:
	...
	if text == "/help":
		_add_local_message(
			"Commands: /connect | /disconnect | /name NEW | /block NAME | /unblock NAME | /blocks | /log | /report NAME | /purgelog | /reset | /syncnow | /status | /listevents | /event | /respond [on|off] | /afk on|off | /traffic [on|off|reset] | /ping",
			Color(0.7, 0.9, 1.0))
		return
	if text == "/ping":
		if not _connected:
			_add_local_message("* /ping: not connected. Wait for the dot to go green.",
				Color(1.0, 0.6, 0.4))
			return
		if _ping_tag != "":
			_add_local_message("* /ping: a previous test is still pending; please wait.",
				Color(0.8, 0.8, 0.4))
			return
		_ping_tag = "PING-%d-%04d" % [Time.get_ticks_msec(), randi() % 10000]
		_ping_t0_ms = Time.get_ticks_msec()
		_ping_timeout_t = (Time.get_ticks_msec() / 1000.0) + 5.0
		# Send directly via _send_raw so it bypasses the local-echo + dedup
		# bookkeeping that _send_chat would normally do -- we WANT to see the
		# server's echo arrive in _handle_incoming.
		_send_raw({
			"type": "chat",
			"username": _username,
			"text": "[selftest %s]" % _ping_tag,
		})
		_add_local_message("* /ping: sent %s, waiting up to 5s for echo..." % _ping_tag,
			Color(0.55, 0.85, 1.0))
		return
	if text.begins_with("/traffic"):
		var targ := text.substr(8).strip_edges().to_lower()
		if targ == "on":
			_net_stats_enabled = true
			_add_local_message("* Traffic reports: ON (every %.0fs)." % _net_stat_interval, Color(0.55, 0.85, 1.0))
		elif targ == "off":
			_net_stats_enabled = false
			_add_local_message("* Traffic reports: OFF.", Color(0.8, 0.8, 0.4))
		elif targ == "reset":
			_net_bytes_in = 0; _net_bytes_out = 0
			_net_msgs_in = 0; _net_msgs_out = 0
			_net_win_bytes_in = 0; _net_win_bytes_out = 0
			_net_win_start_t = Time.get_ticks_msec() / 1000.0
			_add_local_message("* Traffic counters reset.", Color(0.55, 0.85, 1.0))
		else:
			# On-demand report (does not reset the window).
			var now : float = Time.get_ticks_msec() / 1000.0
			var win : float = max(0.001, now - _net_win_start_t)
			var in_rate : float  = _net_win_bytes_in / win
			var out_rate : float = _net_win_bytes_out / win
			_add_local_message("* Relay traffic: in %s/s / out %s/s (%s in, %s out, %d msgs in / %d msgs out total)" % [
				_format_bytes(int(in_rate)),
				_format_bytes(int(out_rate)),
				_format_bytes(_net_bytes_in),
				_format_bytes(_net_bytes_out),
				_net_msgs_in,
				_net_msgs_out,
			], Color(0.55, 0.85, 1.0))
		return
		...
		if _event_registry.is_empty():
			...
			for slug in _event_registry.keys():
				...
				_add_local_message("    [%s] %s.%s  (%s)" % [on_off, e.get("source",""), e.get("category",""), slug],
					Color(0.7, 0.9, 1.0))
		return
	if text == "/event" or text == "/events":
		_start_event_poll()
		return
	if text.begins_with("/respond"):
		var arg := text.substr(8).strip_edges().to_lower()
		if arg == "on":
			_event_respond_enabled = true
			_add_local_message("* /event responder: ON (peers can poll your enabled event templates).",
				Color(0.6, 1, 0.6))
		elif arg == "off":
			_event_respond_enabled = false
			_add_local_message("* /event responder: OFF (your client will ignore /event polls).",
				Color(0.8, 0.8, 0.4))
		else:
			var on_off := "ON" if _event_respond_enabled else "OFF"
			_add_local_message("* /event responder: %s. Use /respond on|off to change." % on_off,
				Color(0.7, 0.9, 1.0))
		return
	if text == "/status":
		var mode_str := ""
...
func _process(delta: float) -> void:
	...
	_tick_afk(delta)
	_tick_net_stats(delta)

	# v1.8.4 /ping timeout: if 5s elapsed with no echo, report failure.
	if _ping_tag != "":
		var now_s : float = Time.get_ticks_msec() / 1000.0
		if now_s >= _ping_timeout_t:
			_add_local_message("* /ping: FAILED -- no echo in 5s. Server may be down or not echoing your own messages.",
				Color(1.0, 0.55, 0.45))
			print("[SafeRelayChat] /ping FAILED tag=", _ping_tag)
			_ping_tag = ""
			_ping_timeout_t = 0.0

	# v1.8.7 /event poll deadline.
	if _event_poll_tag != "":
		var now_pe : float = Time.get_ticks_msec() / 1000.0
		if now_pe >= _event_poll_deadline:
			_finish_event_poll()
	...
	match state:
		WebSocketPeer.STATE_OPEN:
			if not _connected:
				_connected = true
				_reconnect_timer = 0.0
				_add_local_message("* [link] socket OPEN.", Color(0.49, 0.85, 0.34))
				print("[SafeRelayChat] socket OPEN at %.2fs" % (Time.get_ticks_msec()/1000.0))
		...
		WebSocketPeer.STATE_CLOSED:
			if _connected:
				_connected = false
				_add_local_message("* [link] socket CLOSED. Reconnect in %.0fs." % RECONNECT_DELAY, Color(1.0, 0.6, 0.4))
				print("[SafeRelayChat] socket CLOSED at %.2fs" % (Time.get_ticks_msec()/1000.0))
		...
		WebSocketPeer.STATE_CONNECTING:
			# v1.8.1: ensure the hint dot reads as "reconnecting" (amber half),
			# not "offline" (grey) while the handshake is in flight.
			if _reconnect_timer < 0.05:
...
func _drain_packets() -> void:
	while _socket.get_available_packet_count() > 0:
		...
		# Account for incoming bytes (local-only telemetry, never broadcast).
		_net_bytes_in += raw.length()
		_net_win_bytes_in += raw.length()
		_net_msgs_in += 1
...
func _handle_incoming(msg: Dictionary) -> void:
	# v1.8.3: verbose console echo of every legit packet so the player can
	# verify the relay is actually delivering. Never broadcast.
	var _dbg_t := String(msg.get("type", "?"))
	var _dbg_u := String(msg.get("username", ""))
	var _dbg_text := String(msg.get("text", ""))
	if _dbg_text.length() > 60:
		_dbg_text = _dbg_text.substr(0, 60) + "..."
	print("[SafeRelayChat] <- ", _dbg_t, " ", _dbg_u, ": ", _dbg_text)
	...
	if not (t in ["chat", "join", "leave", "server", "event_req", "event_reply"]):
		# v1.8.3: don't escalate to tripwire on unknown types -- some relays
		# include "ping", "pong", "ack", "history" etc. that aren't dangerous.
		# Just drop them with a visible note.
		print("[SafeRelayChat] dropped unknown type '", t, "' from ", _dbg_u)
		return
	...
	if _is_blocked(name_clean):
		...
		print("[SafeRelayChat] dropped: '", name_clean, "' is on your local blocklist")
		return
	...
	match t:
		"chat":
			...
			# v1.8.4 self-test echo: if this is our pending /ping coming
			# back, report round-trip and DON'T render it as a chat line.
			# Check BEFORE dedup because dedup would silently swallow it.
			if _ping_tag != "" and name_clean == _username and text.find(_ping_tag) != -1:
				var rtt_ms : int = Time.get_ticks_msec() - _ping_t0_ms
				_add_local_message("* /ping: OK -- relay echoed in %d ms (round-trip)." % rtt_ms,
					Color(0.55, 1.0, 0.6))
				print("[SafeRelayChat] /ping OK: ", rtt_ms, " ms (tag ", _ping_tag, ")")
				_ping_tag = ""
				_ping_timeout_t = 0.0
				return
			# Dedup: if this looks like the relay echoing our own send back,
			# drop it -- we already rendered it locally on send. Match on
			# username + text within a 6-second window.
			if name_clean == _username:
				var now_s := Time.get_ticks_msec() / 1000.0
				for entry in _recent_self_sends:
					if float(entry.get("t", 0.0)) > now_s - 6.0 \
							and String(entry.get("text", "")) == text:
						if _log != null:
							_log.append(msg, name_clean, text, "echo_dedup")
						return
			...
			if Filters.contains_link_or_path(text):
				...
				_add_local_message("* Dropped message from %s (link/command). Raw: %s" % [name_clean, text.left(40)],
					Color(0.85, 0.5, 0.5))
				print("[SafeRelayChat] dropped: link/command from ", name_clean, ": ", text.left(80))
				# v1.8.3: do NOT bump tripwire on every link. Many casual
				# users paste links and that shouldn't lock out the entire
		...
		"server":
			...
			if Filters.contains_link_or_path(stext):
				...
				print("[SafeRelayChat] dropped server msg with link/command: ", stext.left(80))
				return
			if _filter_in:
		...
		"event_req":
			_handle_event_req(msg, name_clean)
		"event_reply":
			_handle_event_reply(msg, name_clean)
...
func _build_ui() -> void:
	...
	# Try to load RTV's Lora font so the chat panel matches the game's
	# typography (serif, terminal-paper feel). Falls back silently to engine
	# default if the font isn't on disk for some reason.
	if _ui_font == null:
		var f := load("res://Fonts/Lora-Regular.ttf")
		if f is Font:
			_ui_font = f as Font
		var fb := load("res://Fonts/Lora-SemiBold.ttf")
		if fb is Font:
			_ui_font_bold = fb as Font
...
func _build_chat_panel() -> void:
	...
	# RTV-style: pure black background, thin white border, no rounding to
	# match the game's diegetic UI. Opacity from MCM still applies via
	# _chat_panel.modulate.
	...
	_status_label.text = "VOSTOK RELAY CHAT"
	_status_label.add_theme_font_size_override("font_size", 12)
	_status_label.modulate = Color(1.0, 1.0, 1.0, 1.0)
	if _ui_font_bold != null:
		_status_label.add_theme_font_override("font", _ui_font_bold)
	...
	sep.modulate = Color(0.6, 0.6, 0.6, 0.7)
	vbox.add_child(sep)
...
func _build_hint_label() -> void:
	...
	if _ui_font_bold != null:
		btn.add_theme_font_override("font", _ui_font_bold)
	var sb_normal := StyleBoxFlat.new()
	sb_normal.bg_color = Color(0.0, 0.0, 0.0, 0.72)
	sb_normal.border_color = Color(0.95, 0.95, 0.95, 0.85)
	...
	sb_hover.bg_color = Color(0.08, 0.08, 0.08, 0.88)
	btn.add_theme_stylebox_override("hover", sb_hover)
	var sb_press := sb_normal.duplicate() as StyleBoxFlat
	sb_press.bg_color = Color(0.15, 0.15, 0.15, 0.92)
	btn.add_theme_stylebox_override("pressed", sb_press)
	btn.modulate = Color(1.0, 1.0, 1.0, 0.95)
...
func _update_hint_text() -> void:
	...
	# Connection indicator: filled dot = connected, hollow = offline, ! = locked.
	var dot := "[color=#888888]\u25CB[/color]"  # hollow circle, offline
	if _tw_tripped:
		dot = "[color=#ff5050]\u26A0[/color]"  # warning sign, locked out
	elif _connected:
		if _stealth_closed and not _open:
			dot = "[color=#ffd166]\u25CF[/color]"  # yellow filled, stealth
		else:
			dot = "[color=#7ed957]\u25CF[/color]"  # green filled, live
	elif _bg_active:
		dot = "[color=#7ad0ff]\u25CF[/color]"  # blue filled, bg sync
	elif _reconnect_timer > 0.0:
		dot = "[color=#ffd166]\u25D0[/color]"  # half, reconnecting
	if _hint_label:
		var key_part : String
		if _toggle_keycode == EMERGENCY_TOGGLE_KEY:
			key_part = key_str
		else:
			key_part = "%s / %s" % [key_str, emer]
		# Buttons accept plain text only, but we can use modulate per state.
		# Embed the dot as a plain glyph -- color comes from the button's own
		# accent which we set below via add_theme_color_override.
		var glyph := "\u25CB"
		var c := Color(0.55, 0.55, 0.55)
		if _tw_tripped:
			glyph = "\u26A0"
			c = Color(1.0, 0.32, 0.32)
		elif _connected:
			glyph = "\u25CF"
			if _stealth_closed and not _open:
				c = Color(1.0, 0.82, 0.4)
			else:
				c = Color(0.49, 0.85, 0.34)
		elif _bg_active:
			glyph = "\u25CF"
			c = Color(0.48, 0.82, 1.0)
		elif _reconnect_timer > 0.0:
			glyph = "\u25D0"
			c = Color(1.0, 0.82, 0.4)
		_hint_label.text = "%s  RELAY  [%s]" % [glyph, key_part]
		if _hint_label is Button:
			(_hint_label as Button).add_theme_color_override("font_color", c)
			(_hint_label as Button).add_theme_color_override("font_hover_color", c)
			(_hint_label as Button).add_theme_color_override("font_pressed_color", c)
...
func _add_chat_message(username: String, text: String) -> void:
	...
	if _bg_active:
		_bg_collected += 1
	# Color accents: own messages get a cool tag, others a warm one. Body
	# text stays plain white so the panel reads like a printed transcript.
	var tag_color := "#d4c97a"  # warm amber for others
	if u == _scrub_text(_username):
		tag_color = "#7ad0ff"  # cool blue for self
	var bbcode := "[color=%s][b]%s[/b][/color][color=#dddddd]: %s[/color]" % [tag_color, u, t]
...
func _make_label(bbcode: String, color: Color) -> RichTextLabel:
	...
	# Critical: without horizontal expand, fit_content + autowrap collapses
	# the label to ~0 width inside a VBoxContainer and nothing renders.
	lbl.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	lbl.add_theme_font_size_override("normal_font_size", _font_size)
	lbl.add_theme_font_size_override("bold_font_size", _font_size)
	if _ui_font != null:
		lbl.add_theme_font_override("normal_font", _ui_font)
	if _ui_font_bold != null:
		lbl.add_theme_font_override("bold_font", _ui_font_bold)
	lbl.modulate = color
	lbl.text = bbcode
...
func _set_status(text: String) -> void:
	...
	# Refresh the hint button so its connection dot always reflects reality.
	_update_hint_text()
...
func _toggle_chat() -> void:
	...
	if _open:
		...
		# v1.8.4: socket is supposed to be up already (always-on). If for any
		# reason it isn't, kick reconnect off so opening the window doesn't
		# leave you sitting on a stale offline state.
		if not _connected and _has_privacy_ack() and _network_enabled \
				and not (_tw_tripped and _is_lockout_active()):
			_connect_socket()
	else:
		if _game_data:
		...
		# v1.8.1: force hint refresh so the dot reflects "closed+stealth" state
		# (yellow) immediately, instead of carrying over the green-open state.
		_update_hint_text()
		# v1.7.9: NEVER drop the socket on chat close. Doing so silently
		# dropped every message that arrived while the window was hidden.
		# Outgoing is still muted via the _stealth_closed gate in _send_chat
		# and the autobroadcast / mod-event paths.
...
# v1.8.4: simplified -- no more bg-sync handoff (bg-sync is off-default and
# the dual-state mess made connection flow hard to reason about), no more
# mid-session ephemeral re-roll (that only made sense when we dropped on
# close, which we no longer do). Just a guarded _connect_socket().
...
# v1.8.2: periodic local-only traffic report. Numbers come from byte counters
# updated in _send_raw and _drain_packets. Never broadcast.
...
func _tick_net_stats(_delta: float) -> void:
	if not _net_stats_enabled:
		return
	var now : float = Time.get_ticks_msec() / 1000.0
	if now < _net_stat_next_t:
		return
	_net_stat_next_t = now + _net_stat_interval
	# Compute window rate from accumulated window bytes.
	var win : float = max(0.001, now - _net_win_start_t)
	var in_rate : float  = _net_win_bytes_in / win
	var out_rate : float = _net_win_bytes_out / win
	var line := "* Relay traffic: in %s / out %s (%s in, %s out, %d msgs in / %d msgs out total)" % [
		_format_bytes(int(in_rate)) + "/s",
		_format_bytes(int(out_rate)) + "/s",
		_format_bytes(_net_bytes_in),
		_format_bytes(_net_bytes_out),
		_net_msgs_in,
		_net_msgs_out,
	]
	_add_local_message(line, Color(0.55, 0.85, 1.0))
	# Reset the window so the next rate covers only the next interval.
	_net_win_bytes_in = 0
	_net_win_bytes_out = 0
	_net_win_start_t = now
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey:
		...
		if _open:
			# Eat Escape so RTV's pause menu doesn't open behind us. Closes the
			# chat instead. _input fires before _unhandled_input, so the pause
			# menu (which listens via unhandled input) never sees the key.
			if lk == KEY_ESCAPE or pk == KEY_ESCAPE:
				_toggle_chat()
				get_viewport().set_input_as_handled()
			return
...
# Close button [X] in the chat header. v1.8.5: this only HIDES the panel --
# socket stays connected (always-on contract). To actually drop the relay,
# use the kill switch (-) or flip "Mod network" off in MCM. Quitting the
# game also closes the socket cleanly.
func _on_close_button_pressed() -> void:
	print("[SafeRelayChat] close button pressed -> hide panel (socket stays up).")
	if _open:
		_toggle_chat()
...
func _shutdown_socket(reason: String) -> void:
	if _shutdown_done:
		return
	_shutdown_done = true
	print("[SafeRelayChat] shutdown (", reason, ") -- closing socket cleanly")
	# Synchronous leave + close so the relay sees us go even if the OS kills
	# the process a few ms later.
	if is_instance_valid(_socket):
		var st := _socket.get_ready_state()
		if st == WebSocketPeer.STATE_OPEN:
			if not _suppress_jl:
				var payload := JSON.stringify({"type": "leave", "username": _username})
				_socket.send_text(payload)
				_net_bytes_out += payload.length()
				_net_msgs_out += 1
			_socket.close(1000, "client_quit")
		# Pump the socket briefly so the close frame actually leaves before
		# the process exits. Bounded total wait ~150 ms.
		for _i in 15:
...
func _on_app_close_requested() -> void:
	_shutdown_socket("close_requested")

func _on_tree_exiting() -> void:
	_shutdown_socket("tree_exiting")

func _notification(what: int) -> void:
	if what == NOTIFICATION_WM_CLOSE_REQUEST or what == NOTIFICATION_EXIT_TREE or what == NOTIFICATION_PREDELETE:
		_shutdown_socket("notification_%d" % what)


# ---- v1.8.7 /event poll ----------------------------------------------------
# Sender side. Broadcasts an "event_req" packet, opens a 5s reply window.
func _start_event_poll() -> void:
	if not _connected:
		_add_local_message("* /event: not connected. Wait for the dot to go green.",
			Color(1.0, 0.6, 0.4))
		return
	var now : float = Time.get_ticks_msec() / 1000.0
	if now < _event_req_next_t:
		var wait : int = int(ceil(_event_req_next_t - now))
		_add_local_message("* /event: cooldown -- try again in %ds." % wait,
			Color(0.8, 0.8, 0.4))
		return
	if _event_poll_tag != "":
		_add_local_message("* /event: a poll is already in flight.",
			Color(0.8, 0.8, 0.4))
		return
	_event_poll_tag = "EVT-%d-%04d" % [Time.get_ticks_msec(), randi() % 10000]
	_event_poll_replies.clear()
	_event_poll_deadline = now + EVENT_POLL_WINDOW_S
	_event_req_next_t    = now + EVENT_REQ_COOLDOWN_S
	_send_raw({
		"type":     "event_req",
		"username": _username,
		"tag":      _event_poll_tag,
	})
	_add_local_message("* /event: requesting event summaries (waiting %.0fs)..." % EVENT_POLL_WINDOW_S,
		Color(0.55, 0.85, 1.0))

func _finish_event_poll() -> void:
	var n : int = _event_poll_replies.size()
	if n == 0:
		_add_local_message("* /event: no responses. Either no one has events configured, or no one had the responder enabled.",
			Color(0.85, 0.7, 0.45))
	else:
		_add_local_message("* /event: %d response(s):" % n, Color(0.55, 0.85, 1.0))
		for u in _event_poll_replies.keys():
			var line : String = String(_event_poll_replies[u])
			_add_local_message("    %s: %s" % [u, line], Color(0.75, 0.95, 0.75))
	_event_poll_tag = ""
	_event_poll_replies.clear()
	_event_poll_deadline = 0.0

# Receiver side: build and send a one-line summary of OUR enabled events.
func _handle_event_req(msg: Dictionary, name_clean: String) -> void:
	# Never reply to our own request (avoids echo and self-summary).
	if name_clean == _username:
		return
	if not _event_respond_enabled:
		print("[SafeRelayChat] /event: ignoring request from ", name_clean, " (responder disabled).")
		return
	var tag := String(msg.get("tag", ""))
	if tag == "" or tag.length() > 64:
		return
	var now : float = Time.get_ticks_msec() / 1000.0
	if now < _event_resp_next_t:
		print("[SafeRelayChat] /event: rate-limited; ignoring request from ", name_clean)
		return
	# Collect enabled categories with their current templates.
	var enabled_lines : Array[String] = []
	for slug in _event_registry.keys():
		var e : Dictionary = _event_registry[slug]
		if not bool(e.get("enabled", false)):
			continue
		var label := String(e.get("category", slug))
		var tpl   := String(e.get("template", "")).strip_edges()
		if tpl == "":
			enabled_lines.append(label)
		else:
			enabled_lines.append("%s=\"%s\"" % [label, tpl])
	if enabled_lines.is_empty():
		# Nothing to share -- stay silent. This is also a privacy win:
		# someone who has no events enabled is indistinguishable from
		# someone who has the responder turned off. Still arm the cooldown
		# so a flood of event_reqs can't force us to walk the registry
		# every packet.
		_event_resp_next_t = now + EVENT_RESP_COOLDOWN_S
		return
	var payload := " | ".join(enabled_lines)
	# Run through outgoing filters -- same pipeline as normal chat -- so
	# we never leak something a template author tucked in (links, PII).
	payload = Filters.sanitize_unicode(payload, EVENT_REPLY_MAX_LEN)
	if Filters.contains_link_or_path(payload):
		print("[SafeRelayChat] /event: dropped reply (link/command in templates).")
		return
	if _filter_out:
		var scan := Filters.profanity_scan(payload, _extra_terms)
		payload = String(scan.get("clean", payload))
	if payload.length() > EVENT_REPLY_MAX_LEN:
		payload = payload.substr(0, EVENT_REPLY_MAX_LEN - 3) + "..."
	_event_resp_next_t = now + EVENT_RESP_COOLDOWN_S
	_send_raw({
		"type":     "event_reply",
		"username": _username,
...
func _handle_event_reply(msg: Dictionary, name_clean: String) -> void:
	# Ignore stray replies (no in-flight poll, or wrong tag).
	if _event_poll_tag == "":
		return
	if String(msg.get("tag", "")) != _event_poll_tag:
		return
	# Don't accept a reply from ourselves (shouldn't happen, defense in depth).
	if name_clean == _username:
		return
	var payload := Filters.sanitize_unicode(String(msg.get("payload", "")), EVENT_REPLY_MAX_LEN)
	# Same in-bound hardening as chat text.
	if Filters.contains_link_or_path(payload):
		print("[SafeRelayChat] /event: dropped reply from ", name_clean, " (link/command).")
		return
	if _filter_in:
		payload = String(Filters.profanity_scan(payload, _extra_terms).get("clean", payload))
	# Drop empty payloads -- nothing useful to render and a malicious relay
	# could send blank replies under many usernames to push real ones out.
	if payload.strip_edges() == "":
		return
	# Cap dedup dict size so a malicious relay can't grow it unbounded
	# during the 5s collection window. Existing usernames can still update.
	if not _event_poll_replies.has(name_clean) and _event_poll_replies.size() >= EVENT_REPLY_DICT_MAX:
		return
	# Keep one reply per username (last write wins; replies are usually 1-1).
	_event_poll_replies[name_clean] = payload
